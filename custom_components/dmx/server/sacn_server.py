...
class SacnServer:
    ...
    def send_dmx_data(self, universe_id: int, dmx_data: bytearray) -> bool:
        ...
        universe_state.send_task = self.hass.async_create_task(
            self._send_universe_data(universe_id, dmx_data)
        )
    ...
    async def _send_universe_data(self, universe_id: int, dmx_data: bytearray):
        try:
            ...
            log.debug(f"Sent sACN data to universe {universe_id} ({multicast_addr})"
